...
export const AnimationBulkUpload: React.FC = () => {
  ...
  const uploadFiles = async (selectedFiles: File[]) => {
    ...
    const files = [...selectedFiles].sort((a, b) => naturalSort(a.name, b.name))
    ...
    for (const [index, file] of files.entries()) {
      ...
      try {
        const { document } = await findOrCreateVideo(file)
  ...
  return (
    <div style={{ border: '1px solid var(--theme-elevation-150)', marginBottom: '24px', padding: '16px' }}>
      ...
      <p style={{ color: 'var(--theme-elevation-600)', margin: '0 0 12px' }}>
        Select multiple MP4 files. Poster, slug and order are generated automatically.
      </p>
      <input
        accept="video/mp4"
        disabled={running}
